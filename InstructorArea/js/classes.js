...
function displayList() {
    ...
    $.ajax({
        url: "AJAX/displayClassPagination.php",
        type: "POST",
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response === "fail") {
                        ...
                        Toast.fire({
                            icon: 'success',
                            html: '<b>Successful</b><br/>Removed the class.'
                        })
...
function downloadXLSX(){
    ...
    if (table && table.length) {
        $(table).table2excel({
                exclude: ".noExl",
                name: "Classes",
                filename: "Classes" + new Date().toISOString().replace(/[\-\:\.]/g, "") + ".xls",
                fileext: ".xls",
                exclude_img: true,
                exclude_links: true,
                exclude_inputs: true,
