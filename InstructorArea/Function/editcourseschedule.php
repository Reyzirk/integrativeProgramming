...
if (empty($_GET["id"])){
    ...
    $retrievedSchedule = $scheduleDB->details($id);
    if (!empty($retrievedSchedule)){
        $storedValue["scheduleID"] = $id;
        $storedValue["classID"] = $retrievedSchedule->classID;
        $storedValue["courseCode"] = $retrievedSchedule->courseCode;
        $storedValue["instructor"] = $retrievedSchedule->instructor;
        $datetime = new DateTime($retrievedSchedule->timeStart);
        $storedValue["timeStart"] = $datetime->format("H:i");
        $storedValue["duration"] = $retrievedSchedule->duration;
        $storedValue["dayOfWeek"] = $retrievedSchedule->day;
        $storedValue["classType"] = $retrievedSchedule->classType;
    }else{
        $_SESSION["errorLog"] = "noid";
        header('HTTP/1.1 307 Temporary Redirect');
        header('Location: classes.php');
...
if (isset($_POST["formDetect"])){
    ...
    if (empty($error)){
        $newSchedule = new CourseSchedule($storedValue["scheduleID"],$storedValue["courseCode"],$storedValue["classID"],$storedValue["instructor"],$storedValue["timeStart"],$storedValue["duration"],$storedValue["classType"],$storedValue["dayOfWeek"]);

        $scheduleDB = new CourseScheduleDB();
        $scheduleDB->update($newSchedule);
        $_SESSION["modifyLog"] = "editcourseschedule";
        header('HTTP/1.1 307 Temporary Redirect');
        header('Location: courseschedule.php?id='.$storedValue["classID"]);
